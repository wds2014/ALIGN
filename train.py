...
def extend_cfg(cfg):
    ...
    # Config for MMP
    cfg.TRAINER.MMP = CN()
    cfg.TRAINER.MMP.N_CTX = 2  # number of context vectors
    cfg.TRAINER.MMP.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.MMP.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.MMP.TEXT_PROMPT_DEPTH = 1  # Max 12, minimum 0, for 1 it will act as shallow MMP (J=1)
    cfg.TRAINER.MMP.VISION_PROMPT_DEPTH = 1  # Max 12, minimum 0, for 1 it will act as shallow MMP (J=1)
    cfg.TRAINER.MMP.TEXT_PROMPT_NUMBER = 4   # number of to be learned language prompts
    cfg.TRAINER.MMP.VISION_PROMPT_NUMBER = 4  # number of to be learned vision prompts
    cfg.TRAINER.MMP.HIERARCHICAL = True
    cfg.TRAINER.MMP.USECT = True
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
...
def setup_cfg(args):
    ...
    if cfg.DATASET.SUBSAMPLE_CLASSES == 'all':   ## few shot setting
